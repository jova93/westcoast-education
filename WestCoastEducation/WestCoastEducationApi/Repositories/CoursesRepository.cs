...
public class CoursesRepository
{
    ...
    public CoursesRepository(
        IOptions<WestCoastEducationStoreDatabaseSettings> westCoastEducationStoreDatabaseSettings)
    {
        var mongoClient = new MongoClient(
            westCoastEducationStoreDatabaseSettings.Value.ConnectionString);

        var mongoDatabase = mongoClient.GetDatabase(
            westCoastEducationStoreDatabaseSettings.Value.DatabaseName);

        _coursesCollection = mongoDatabase.GetCollection<Course>(
            westCoastEducationStoreDatabaseSettings.Value.CoursesCollectionName);

        _studentsCollection = mongoDatabase.GetCollection<Student>(
            westCoastEducationStoreDatabaseSettings.Value.StudentsCollectionName);
    }
    ...
    // Get a specific course based on an Id.
    ...
    // Update a specific course based on an Id.
    ...
    // Remove a specific course based on an Id.
    ...
    public async Task<List<Student>?> GetParticipantsAsync(string id)
    {
        ...
        // All students that are registered.
